...
class REAPI:
    ...
    def render(self, game_surface):
        ...
        if self.map_index>-1:
            ...
            # IMPORTANT WE ARE NOT DOING THIS ANYMORE. WE ARE DOING A BIT MORE LIKE QUAKE. SO WHAT WE DO IS READ THE OBJ OR BSP OF THE ENTIRE WORLD, 
            # BUT KEEP IN MIND THAT WE NEED TO ESSENTIALLY BEFORE STARTING A LEVEL SEND THE PLAYER TO A PLACE OR DICTATE WHERE TO PLACE THE PLAYER
            # THIS SHOULD BE IMPORTANT AND REQUIRED WHEN LOADING A LEVEL
            # Also the skybox has to be set up manually

            # You could also turn off gravity etc, which is the player falling and everything falling, turn on flying all that good stuff. You only draw the first 256 quads btw tho this can change
            # read the maps data of where the player is and update the player position to that

            # So we simply need to update it to that for every vertex. we can simply do it before we render that specific quad. Most importantly, we should first walk through the tree
            # then after we have decided the ordering, we go for the ordering front to back
            # the put the vertices where they need to be, and then we texture the quad
            # Then we render the floor and ceiling whichever one is best in the order you believe should be ( i think bottom to top for floor idk if we are at the right eyelevel. )
            # and then after you have drawn the walls all of them, we go to the floor and we have to look at which sector is visible, and i guess we draw that if its on screen, and we draw them top to bottom
