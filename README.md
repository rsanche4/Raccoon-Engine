...
The Raccoon Engine is a lightweight game engine inspired by the simplicity and flexibility of Build Engine-style architectures and grid stepping logic of raycasters. It combines a Java core with a Lua frontend for easy scripting and rapid prototyping. Although it has been built for creating boomer style shooters, the engine is so general and powerful it can also be used for 2D game development as well!
...
Scripting your game requires an understanding of programming, and how the overall architecture of the engine interacts. This readme is a good starting point. Once you get a general sense of what the engine is good for, I recommend learning Java and Lua. Java will mainly be used for developing your API functions, and Lua will be the scripts that call those functions and use the engine to achieve all sorts of complex behaviors. If you come from an RPG Maker background, some of the concepts should be intuitive as I was heavily inspired by the events system of RPG Maker. The overall idae is this: every .lua script has to be added to the global scripts array using the "add script" API calls. Unless a script decides to end itself using the "endme" api call, a script will simply run every frame of the game in the order they were added to the array. Scripts have their ids which are provided when they run as arguments, and one can access such ids to end other scripts from another script. Another important concept for scripts is the concept of behavior scripts. These are slightly different from regular scripts in the sense that they are usually attached to a sprite. Sprites are essentially in-game dynamic entities. So these would be your NPCs, enemies, moving objects, etc. When you add a sprite to the world, you also need to attach a behavior script to that sprite. These behavior script can access things like the sprite location in world, its name, id, and things that would be rather complicated to access with using basic scripts. As such, this system allows you to control pretty much anything and everything in the world with minimal complexity, and gives you full power over what you want to do. This is so powerful in fact that you do not have to make a boomer shooter game. You could in fact ditch 3D entirely and use the engine solely for 2D games as well. I have created a number of these and as such, the engine really can work as a general purpose engine for making any type of games as well! 
...
Because of the column-based rendering, true looking up and down would cause visual distortions. The engine is locked to horizontal rotation only (yaw), giving you 1 degree of rotational freedom. Your view always remains level with the horizon. However, this has been faked just like the original Duke Nukem 3D by simply changing that horizon look up and down. This is what causes the classic distortion that was present in the shooter games of the 90s when you look up or down too much.
